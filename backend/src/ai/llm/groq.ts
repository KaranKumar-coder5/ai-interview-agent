...
export class GroqProvider implements LLMProvider {
  ...
  constructor(
    ...
    this.apiKey = key;
    this.modelName =
      modelName || (config.provider === "groq" ? config.model : "openai/gpt-oss-120b");
    this.timeoutMs = timeoutMs || config.timeoutMs || 5000;
    this.baseUrl = baseUrl || "https://api.groq.com/openai/v1/chat/completions";
  }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (process.env.NODE_ENV !== "production") {
          console.error(
            `[GroqProvider Dev Error] Status: ${response.status} ${response.statusText} | Model: ${this.modelName} | Details: ${errSnippet || "N/A"}`,
          );
        }

        const statusMsg = `Groq API returned HTTP error ${response.status} (${response.statusText})${
      ...
      if (!text || typeof text !== "string" || text.trim() === "") {
        if (process.env.NODE_ENV !== "production") {
          console.error(
            `[GroqProvider Dev Error] Empty completion content returned from model ${this.modelName}.`,
          );
        }
      ...
      if (err instanceof Error && err.name === "AbortError") {
        if (process.env.NODE_ENV !== "production") {
          console.error(
            `[GroqProvider Dev Error] Timeout after ${this.timeoutMs}ms | Model: ${this.modelName}`,
          );
        }
      ...
      if (process.env.NODE_ENV !== "production") {
        ...
        if (!msg.includes("[GroqProvider Dev Error]")) {
          console.error(
            `[GroqProvider Dev Error] Request Failure: ${msg} | Model: ${this.modelName}`,
          );
        }
      }
  ...
  async analyzeAnswer(
    ...
    if (!validated) {
      if (process.env.NODE_ENV !== "production") {
        console.error(
          `[GroqProvider Dev Error] Malformed structured answer analysis JSON schema from model ${this.modelName}.`,
        );
      }
      throw new Error("Groq returned malformed structured answer analysis JSON.");
    }
  ...
  async generateFollowUp(
    ...
    if (!validated) {
      if (process.env.NODE_ENV !== "production") {
        console.error(
          `[GroqProvider Dev Error] Malformed structured follow-up JSON schema from model ${this.modelName}.`,
        );
      }
      throw new Error("Groq returned malformed structured follow-up JSON.");
    }
  ...
  async generateFeedback(session: Session): Promise<Feedback> {
    ...
    if (!validated) {
      if (process.env.NODE_ENV !== "production") {
        console.error(
          `[GroqProvider Dev Error] Malformed structured feedback JSON schema from model ${this.modelName}.`,
        );
      }
